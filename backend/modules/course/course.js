...
const validator = (course) => {
  ...
  if(!validation.mininteger(course.DepartmentId, 1, "Department Id")) return validation.getError;
 
  if(!validation.mininteger(course.UniqueId, 4, "Unique Id")) return validation.getError;
  
  if(!validation.minlength(course.Code, 2, "Code")) return validation.getError;
  
    if(!validation.mininteger(course.Units, 1, "Units")) return validation.getError;
 
    if(!validation.mininteger(course.CourseLevel, 1, "Course Level")) return validation.getError;
 
    if(!validation.mininteger(course.CourseSemester, 1, "Course Semester")) return validation.getError;
  
    if(!validation.mininteger(course.Status, 1, "Status")) return validation.getError;
