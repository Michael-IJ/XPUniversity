...
const addLecturer = async (lecturer) => {
  try {
    const result = await pool.query('INSERT INTO "AcadSchema"."Lecturer" ("FirstName", "Surname", "OtherName", "StaffId", "DepartmentId", "Status") VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',  [lecturer.FirstName, lecturer.Surname, lecturer.OtherName, lecturer.StaffId, lecturer.DepartmentId, lecturer.Status]);
    if (result.rows.length > 0) {
      return result.rows[0];
    }
    // return [];
...
const editLecturer = async (lecturer) => {
  try {
    console.log("working");
    const result = await pool.query('UPDATE "AcadSchema"."Lecturer" SET "FirstName" = $1, "Surname" = $2, "OtherName" = $3, "StaffId" = $4, "DepartmentId" = $5, "Status" = $6 WHERE "LecturerId" = $7 RETURNING *',[lecturer.FirstName, lecturer.Surname, lecturer.OtherName, lecturer.StaffId, lecturer.DepartmentId, lecturer.Status, lecturer.LecturerId]);
    if (result.rows.length > 0) {
      return result.rows[0];
    }
    return null;
