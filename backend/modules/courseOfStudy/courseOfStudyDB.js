...
const addCourseOfStudy = async (courseOfStudy) => {
  try {
    ...
    const result = await pool.query('INSERT INTO "AcadSchema"."CourseOfStudy" ("DepartmentId", "Name", "ShortName", "UniqueId", "Award", "Duration", "RequiredCreditUnit", "Advisor", "Status") VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *',  [courseOfStudy.DepartmentId, courseOfStudy.Name, courseOfStudy.ShortName, courseOfStudy.UniqueId, courseOfStudy.Award, courseOfStudy.Duration, courseOfStudy.RequiredCreditUnit, courseOfStudy.Advisor, courseOfStudy.Status]);
    if (result.rows.length > 0) {
      return result.rows[0];
    }
    return null;
...
const editCourseOfStudy = async (courseOfStudy) => {
  try {
    const result = await pool.query('UPDATE "AcadSchema"."CourseOfStudy" SET "DepartmentId" = $1, "Name" = $2, "ShortName" = $3, "UniqueId" = $4, "Award" = $5, "Duration" = $6, "RequiredCreditUnit" = $7, "Advisor" = $8, "Status" = $9 WHERE "CourseOfStudyId" = $10 RETURNING *',  [courseOfStudy.DepartmentId, courseOfStudy.Name, courseOfStudy.ShortName, courseOfStudy.UniqueId, courseOfStudy.Award, courseOfStudy.Duration, courseOfStudy.RequiredCreditUnit, courseOfStudy.Advisor, courseOfStudy.Status, courseOfStudy.CourseOfStudyId]);
    if (result.rows.length > 0) {
      return result.rows[0];
    }
    return null;
